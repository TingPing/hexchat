...
win32_tint (GtkXText *xtext, GdkImage *img, int width, int height)
{
	...
	int x, y;
	GdkPixmap *pix;
	GdkVisual *visual = gdk_drawable_get_visual (gtk_widget_get_window (GTK_WIDGET (xtext)));
	guint32 pixel;
	int r, g, b;

	if (gdk_image_get_depth (img) <= 14)
	{
		...
		for (y = 0; y < height; y++)
		{
			for (x = 0; x < width; x++)
			{
				if (gdk_image_get_depth (img) == 1)
				{
					pixel = (((guchar *) img->mem)[y * img->bpl + (x >> 3)] & (1 << (7 - (x & 0x7)))) != 0;
					goto here;
				}

				if (gdk_image_get_depth (img) == 4)
				{
...
here:
	...
	} else
	{
		shade_image (visual, img->mem, img->bpl, img->bpp, width, height,
						 ...
						 xtext->palette[XTEXT_BG], gdk_visual_get_depth (visual));
	}
...
gtk_xtext_load_trans (GtkXText * xtext)
{
	...
	hwnd = GDK_WINDOW_HWND (gtk_widget_get_window (GTK_WIDGET (xtext)));
	hdc = GetDC (hwnd);
	PaintDesktop (hdc);
	ReleaseDC (hwnd, hdc);

	gdk_drawable_get_size (gtk_widget_get_window (GTK_WIDGET (xtext)), &width, &height);
	img = gdk_drawable_get_image (gtk_widget_get_window (GTK_WIDGET (xtext)), 0, 0, width+128, height);
	xtext->pixmap = win32_tint (xtext, img, gdk_image_get_width (img), gdk_image_get_height (img));
