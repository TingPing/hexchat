...
static void dcc_read (GSocketClient *client, GAsyncResult *res, struct DCC *dcc);
static gboolean dcc_read_ack (GIOChannel *source, GIOCondition condition, struct DCC *dcc);
static void dcc_connection_ready (GSocketClient *client, GAsyncResult *res, struct DCC *dcc);
...
static void
dcc_create_connection (struct DCC *dcc)
{
	...
	g_socket_client_connect_to_host_async (client, address, dcc->port,
					NULL, (GAsyncReadyCallback)dcc_connection_ready, dcc);
	g_free (address);
}
...
dcc_send_ack (struct DCC *dcc)
{
	...
	guint32 pos = htonl (dcc->pos & 0xffffffff); /* send in 32-bit big endian */

	stream = g_io_stream_get_output_stream (G_IO_STREAM(dcc->conn));
	g_output_stream_write (stream, (char *) &pos, 4, NULL, NULL);
}

static void
dcc_read (GSocketClient *client, GAsyncResult *res, struct DCC *dcc)
{
	char *old;
	int n;
...
static void
dcc_connection_ready (GSocketClient *client, GAsyncResult *res, struct DCC *dcc)
{
	...
	dcc->conn = g_socket_client_connect_to_host_finish (client, res, NULL);
	if (!dcc->conn)
	{
		dcc->dccstat = STAT_FAILED;
		fe_dcc_update (dcc);
	...
	address = g_inet_address_to_string (dcc->addr);
	snprintf (host, sizeof host, "%s:%d", address, dcc->port);
	g_free (address);
	
	in_stream = g_io_stream_get_input_stream (G_IO_STREAM(dcc->conn));
	out_stream = g_io_stream_get_output_stream (G_IO_STREAM(dcc->conn));

	switch (dcc->type)
	{
	case TYPE_RECV:
		g_input_stream_read_async (in_stream,
						dcc->read_buf, sizeof(dcc->read_buf), G_PRIORITY_LOW,
						NULL, (GAsyncReadyCallback)dcc_read, dcc);
	...
	case TYPE_SEND:
		...
		if (dcc->fastsend)
		{
			g_output_stream_write_async (out_stream,
							dcc->read_buf, sizeof(dcc->read_buf), G_PRIORITY_LOW,
							NULL, (GAsyncReadyCallback)dcc_send_data, dcc);
		}
		g_input_stream_read_async (in_stream,
						dcc->read_buf, sizeof(dcc->read_buf), G_PRIORITY_LOW,
						NULL, (GAsyncReadyCallback)dcc_read_ack, dcc);
#if 0
		if (dcc->fastsend)
		{
	...
	case TYPE_CHATRECV:	/* normal chat */
		g_input_stream_read_async (in_stream,
						dcc->read_buf, sizeof(dcc->read_buf), G_PRIORITY_DEFAULT,
						NULL, (GAsyncReadyCallback)dcc_read_chat, dcc);
...
static void
dcc_accept (GSocketListener *listener, GAsyncResult *res, struct DCC *dcc)
{
	GSocketConnection *conn;
	GError *error = NULL;
	
	conn = g_socket_listener_accept_finish (listener, res, NULL, &error);
	if (error)
	{
...
dcc_get_my_address (void)	/* the address we'll tell the other person */
{
	...
	if (prefs.hex_dcc_ip[0])
	{
		addr = g_inet_address_new_from_string (prefs.hex_dcc_ip);
	}
...
dcc_listen_init (struct DCC *dcc, session *sess)
{
	...
	/* TODO: listen on prefs.local_ip */

	/* Listen within set port-range */
	if (prefs.hex_dcc_port_first > 0)
	{
		int i = prefs.hex_dcc_port_first;
		while (prefs.hex_dcc_port_last > i)
		{			
	...
	{
		port = g_socket_listener_add_any_inet_port (listener, NULL, NULL);
	}

	if (!port)
	{
		PrintText (sess, "Failed to bind to any port.\n");
		return FALSE;
	}
	...
	g_socket_listener_accept_async (listener, NULL, (GAsyncReadyCallback)dcc_accept, dcc);
...
static struct DCC *
dcc_add_chat (session *sess, char *nick, int port, guint8 addr, int pasvid)
{
	struct DCC *dcc;
	...
	if (dcc)
	{
		...
		dcc->dccstat = STAT_QUEUED;
		dcc->addr = g_inet_address_new_from_bytes (addr, G_SOCKET_FAMILY_IPV4);
		dcc->port = port;
		dcc->pasvid = pasvid;
		dcc->nick = strdup (nick);
		dcc->starttime = time (0);
...
static struct DCC *
dcc_add_file (session *sess, char *file, DCC_SIZE size, int port, char *nick, guint8 addr, int pasvid)
{
	struct DCC *dcc;
	char tbuf[512];
